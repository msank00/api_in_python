...
def apiValidate():
    try:
        ...
        if request.headers["Content-Type"] == "application/json":
            responceJsonObj, jsonDoc, warJsonArray, resCode = validate(
                paramStr, responceJsonObj
            )
            count = 0
            ...
            return (
                json.dumps(responceJsonObj),
                ResponseCode.UnsupportedMediaType.value,
            )
...
def apiPredict():
    try:
        ...
        if request.headers["Content-Type"] == "application/json":
            responceJsonObj, jsonArray, warJsonArray, resCode = validate(
                paramStr, responceJsonObj
            )
        ...
        if len(reqFieldErr) > 0:
            ...
            if len(warFields) > 0:
                for fld in warFields:
                    errfieldList.append(
                        {"FIELD": fld, "WARNING": warFields[fld]}
                    )
                    app.logger.warning(
                        "WARNING: FIELD %s: %s " % (fld, warFields[fld])
                    )
...
def pridict(jsonArray, responceJsonObj, warJsonArray):
    ...
    pridictDf = pd.read_csv(
        StringIO(csvFileObj), header=0, dtype={"Age": np.float64}
    )
    ...
    pridictDf["Title"] = pridictDf["Title"].replace(
        [
            "Lady",
            "Countess",
            "Capt",
    ...
    # Mapping Sex
    pridictDf["Sex"] = pridictDf["Sex"].map(
        {"female": 0, "male": 1}
    )  # .astype(int)
    # Mapping titles
    ...
    # Mapping Embarked
    pridictDf["Embarked"] = pridictDf["Embarked"].map(
        {"S": 0, "C": 1, "Q": 2}
    )  # .astype(int)
    #  Mapping Fare
    pridictDf.loc[pridictDf["Fare"] <= 7.91, "Fare"] = 0
    pridictDf.loc[
        (pridictDf["Fare"] > 7.91) & (pridictDf["Fare"] <= 14.454), "Fare"
    ] = 1
    pridictDf.loc[
        (pridictDf["Fare"] > 14.454) & (pridictDf["Fare"] <= 31), "Fare"
    ] = 2
    ...
    pridictDf.loc[
        (pridictDf["Age"] > 16) & (pridictDf["Age"] <= 32), "Age"
    ] = 1
    pridictDf.loc[
        (pridictDf["Age"] > 32) & (pridictDf["Age"] <= 48), "Age"
    ] = 2
    pridictDf.loc[
        (pridictDf["Age"] > 48) & (pridictDf["Age"] <= 64), "Age"
    ] = 3
    ...
    if df[col].dtype == np.float64 or df[col].dtype == np.int64:
        for index, row in df.iterrows():
            if np.isnan(row[col]):
                ...
                app.logger.info(
                    "getting mean value from training data set for the Nan field %s as '%s'"
                    % col,
                    meanValue,
                )
